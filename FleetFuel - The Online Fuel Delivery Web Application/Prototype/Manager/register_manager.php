...
if (isset($_POST['add_user'])) {
    ...
    if (mysqli_num_rows($check_fullname) > 0) {
        $message = "Company Already Exist Please Enter Some Other name";
    } else if (mysqli_num_rows($check_username) > 0) {
        $message = "Username Already Exist";
    } else if (mysqli_num_rows($check_email) > 0) {
        $message = "Email  Already Exist";
    } else if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $error =  "Your email adress is valid";
        ...
        if (mysqli_query($conn, $query)) {
            $style = "alert-success";
            $message = "Tour Company Registered Successfully<a href='index.php'>Go To Login Page</a>";
        }
    } else {
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <table class="table table-borderless">
                                        ...
                                        <tr>
                                            <td></td>
                                            <td><input type="submit" name="add_user" class="cust-btn-in btn btn-block" value="Register As A Manager" /></td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
